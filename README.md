...
6. **Tailoring for a specific job?** Drop the posting in `info/job_listings/`, tailor `resume.tex`, then run `./generate_tailored_resume_copy.sh CompanyName` to save a company-specific copy under `custom_resumes/`. See `CLAUDE.md`'s "Tailoring workflow for a job listing" for the full flow.
